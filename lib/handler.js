...
module.exports = class Handler {
	...
	static async destroy() {
		for (const def of Handler.pools) {
			if (def.instance) {
				await def.instance.close(true);
				delete def.instance;
			}
		}
	...
	async #initBrowser() {
		const expath =
			(Handler.executable && await which(Handler.executable, { nothrow: true }))
			?? await which(Handler.browser, { nothrow: true })
		...
		Handler.#browser = await playwright[Handler.browser].launch(opts);
	}
