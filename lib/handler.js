...
module.exports = class Handler {
	...
	async runMethod(phase, req, res) {
		...
		const browser = settings.browser ?? this.opts.browser;
		...
		await page.setViewport({
			width: 640,
			height: 480,
			deviceScaleFactor: settings.devicePixelRatio ?? this.opts.devicePixelRatio
		});
		...
		for (const plugin of settings.plugins) {
			const fn = this.opts.plugins[plugin];
			if (!fn) {
