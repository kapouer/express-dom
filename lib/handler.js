...
class Browsers {
	...
	async get(browser) {
		if (!this.#browsers.has(browser)) {
			...
			this.#browsers.set(browser, puppeteer.launch(opts));
		}
		const inst = await this.#browsers.get(browser);
		if (!inst.connected) {
			console.warn("express-dom browser disconnected");
			try {
				await inst.close();
			} catch {
				// pass
...
class PoolFactory {
	...
	async create() {
		...
		context.page = await this.#page(context);
		return context;
	}
	...
	async reset(context) {
		try {
			const previous = await context.cookies();
			await context.page.deleteCookie(...previous.map(cookie => {
				return {
					name: cookie.name
				};
			}));
...
module.exports = class Handler {
	...
	static init() {
		if (!this.#browsers) this.#browsers = new Browsers(Handler.defaults);
	}
	...
	async release(context) {
		const { pool } = context;
		delete context.pool;
		context.page?.removeAllListeners();
		try {
	...
	async runMethod(phase, req, res) {
		...
		const context = await this.acquire(browser, devicePixelRatio);
		const { page } = context;
