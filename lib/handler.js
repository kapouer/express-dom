...
module.exports = class Handler {
	...
	async runMiddleware(req, res, next) {
		...
		const source = req.get(header.name);

		if (this.#helper) {
			await this.#helper({
				phase: source, location, online, offline, plugins
			}, req, res);
		...
		if (source == null) {
			// run online else jump to offline phase
			if (runOn) {
				phase = header.online;
				opts = online;
			} else if (runOff) {
				phase = header.offline;
				opts = offline;
				csp = online.enabled ? online.policies : null;
			} else {
				phase = false;
			}
		} else if (source === header.offline) {
			// run source phase for offline phase
			phase = false;
			csp = offline.policies;
		} else if (source == header.online) {
			// run offline or directly source phase, for online phase
			csp = online.policies;
			if (runOff) {
				phase = header.offline;
				opts = offline;
		...
		if (csp) {
			res.set('Content-Security-Policy', buildPolicies(csp));
		}
		res.vary(header.name);
		if (phase === false) {
			next();
		} else {
			if (Handler.debug) opts.timeout = 0;
			await this.runMethod(phase, plugins, location, opts, req, res);
		}
	}

	async runMethod(phase, plugins, loc, settings, req, res) {
		...
		await page.route(loc.href, route => {
			const headers = {
				...route.request().headers(),
				[Handler.header.name]: phase
			};
