...
module.exports = class Handler {
	...
	async runMethod(phase, req, res) {
		...
		const cliOpts = {
			...
			script: true,
			link: settings.visible,
			img: settings.visible
		};

		await initScripts(page, [
			track ? [tracker, cliOpts] : null,
			styles.length > 0 ? [initStyles, styles.join('\n')] : null
		].concat(scripts));

		const cliTrack = new Deferred();
		cliTrack.then(() => {
			cliTrack.done = true;
		});
		const reqTrack = new RequestTracker(page, cliTrack);
		...
		try {
			...
			debug("page loaded");
			if (track) {
				page.evaluate(id => window[id], cliOpts.id).then(cliTrack.resolve);
			} else {
				cliTrack.resolve();
			}

			const timeTrack = new Deferred();
			timeTrack.to = setTimeout(timeTrack.resolve, timeout);

			const closeTrack = new Deferred();
			page.on('close', closeTrack.resolve);

			const event = await Promise.race([
				reqTrack.then(() => 'idle'),
				timeTrack.then(() => 'timeout'),
				closeTrack.then(() => 'close')
			]);
			clearTimeout(timeTrack.to);
			if (event == "timeout") {
