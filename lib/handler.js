...
module.exports = class Handler {
	...
	async runMethod(method, settings, req, res) {
		...
		if (mSettings.verbose) page.on('console', msg => {
			...
			if (type) {
				const text = msg.text();
				// ignore errors from our own CSP header
				if (text.startsWith("Refused to ") > 0 && text.indexOf("Content Security Policy") > 0) return;
				// eslint-disable-next-line no-console
				console[type](text);
			}
		});
		...
		try {
			...
			const event = await Promise.race([
				...
				new Promise(resolve => {
					setTimeout(() => resolve("timeout"), mSettings.timeout);
				})
			]);
			if (event == "timeout") {
