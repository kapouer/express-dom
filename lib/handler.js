...
module.exports = class Handler {
	...
	async runMiddleware(req, res, next) {
		...
		if (this.#helper) {
			await this.#helper({ location, online, offline }, req, res);
		...
		if (phase === null) {
			...
			policies = offline.policies;
		}
		if (policies) {
			res.set('Content-Security-Policy', buildPolicies(policies));
		}
		if (!settings) {
			return next();
		} else {
			location.searchParams.set('develop', param);
			await this.runMethod(location, settings, req, res);
		}
	}

	async runMethod(loc, settings, req, res) {
		...
		const {
			plugins, scripts, styles, timeout, referer
		} = settings;

		page.location = loc;

		for (const plugin of plugins) {
			const fn = Handler.plugins[plugin];
			if (!fn) {
		...
		page.on('crash', defer.reject);
		page.on('pageerror', defer.reject);
