...
class PoolFactory {
	...
	async create() {
		const context = await this.#browser.createBrowserContext(this.#opts);
...
module.exports = class Handler {
	...
	constructor(conf = {}) {
		Object.assign(puppeteer.Page.prototype, EventEmitter.prototype);
	...
	async #initBrowser(browser) {
		...
		Object.assign(opts, Handler.defaults.browsers[browser]);
	...
	async acquire(browser) {
		...
		if (!browsers.has(browser)) {
			browsers.set(browser, await this.#initBrowser(browser));
		}
		const pools = Handler.#pools;
		if (!pools.has(browser)) {
			pools.set(browser, new Pool(
				new PoolFactory(browsers.get(browser), {
					...Handler.defaults.page
				}), { ...Handler.defaults.pool }
			));
		}
		const pool = pools.get(browser);
		const page = await pool.acquire();
		page.pool = pool;
		const { regpol } = Handler.defaults.browsers[browser];
		page.isCSPError = str => {
	...
	async runMethod(phase, req, res) {
		...
		const browser = settings.browser ?? Handler.defaults.browser;
		if (!Handler.defaults.browsers[browser]) {
			throw new Error("Unknown browser: " + browser);
		}
		const page = await this.acquire(browser);
		await page.setViewport({
			width: 640,
			height: 480,
			deviceScaleFactor: settings.devicePixelRatio ?? Handler.defaults.devicePixelRatio
		});
