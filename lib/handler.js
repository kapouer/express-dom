...
class PoolFactory {
	...
	async destroy(page) {
		try {
			...
			console.error("Error destroying browserContext", err);
		}
	}
...
module.exports = class Handler {
	...
	async release(page) {
		...
		try {
			if (!page.isClosed()) await Promise.all(evals.map(ev => {
				return page.removeScriptToEvaluateOnNewDocument(ev.identifier);
			}));
		} catch {
			try {
