...
exports.view = async function view(mw, settings, req, res) {
	...
	if (req.protocol == "about:" && req.path == "blank") {
		return update('<html/>');
	} else if (
...
function tryRoot(views, view) {
	...
	return readFile(path, { encoding: 'utf-8' }).catch(() => {
...
exports.prioritize = function(mw, settings, { xhr }) {
	if (!settings.priority) settings.priority = 0;
	if (xhr) settings.priority++;
};

exports.develop = function (mw, settings, { query }) {
