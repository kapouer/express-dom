...
exports.view = function view(mw, settings, request, response) {
	...
	const view = settings.view || request.path;
	...
	let views = settings.views || request.app && request.app.get('views') || [];
...
function fetchUrl(url) {
	const obj = typeof url == "string" ? new URL(url) : url;
	...
	obj.href = obj.toString();

	debug("requesting", obj.href);
	return new Promise((resolve, reject) => {
		const req = (obj.protocol == "https:" ? https : http).request(obj, (res) => {
			if (res.headers.location) {
				const location = new URL(res.headers.location, obj.href);
				try {
					req.abort();
				} catch(ex) {
					// ignore
				}
				location.redirects = (obj.redirects || 0) + 1;
