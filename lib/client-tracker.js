module.exports = function (trackerOpts) {
	...
	class AsyncTracker {
		...
		constructor(opts) {
			...
			for (const context of contexts) {
				...
				if (typeof trackFn == "function") {
					...
					console.error("Option track is expected to be a function or a boolean", trackFn);
				} else {
					...
					this.#handleTimeout(track, opts.timeout);
		...
		#handleTimeout({ native, context }, timeout) {
			const it = this;
			native.setTimeout = context.setTimeout;
			context.setTimeout = function (fn, to) {
				if (timeout && to && to > timeout) to = timeout;
				if (typeof fn != "function") {
		...
		#done(id) {
			...
			if (this.#sources?.size === 0) {
				if (!this.debug) this.#destroy();
				this.debug?.("idle", this.#idles++);
				this.#resolve("idle");
			}
		}
