module.exports = function (trackerOpts) {
	...
	class AsyncTracker {
		...
		#handleNodes({ context }) {
			...
			if (this.#link) guard(HTMLLinkElement, 'href');
			if (this.#script) guard(HTMLScriptElement, 'src');

			const check = node => {
				if (node.nodeType != Node.ELEMENT_NODE) return;
				const tag = node.nodeName;
				if (tag == "SCRIPT") {
					return this.#script && node.src && (!node.type || node.type == "text/javascript" || node.type == "module") && alls.has(node);
				} else if (tag == "LINK") {
					return this.#link && node.href && node.rel == "stylesheet" && alls.has(node);
				} else if (tag == "IMG") {
					return this.#img && node.src;
				}
			}
	...
	const tracker = new AsyncTracker(trackerOpts);
