...
Pool.prototype.acquire = function(cb) {
	...
	if (page) {
		this.release(page, true, function() {
			page.locked = true;
...
Pool.prototype.release = function(page, quick, cb) {
	debug("release page", quick, page.uri);
	page.locked = true;
	page.releaseTime = Date.now();
	(quick ? page.reset : page.unload)(function(err) {
		page.locked = false;
		cb(err);
