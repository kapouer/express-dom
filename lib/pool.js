var WebKit = require('webkitgtk');
var debug = require('debug')('express-dom');

module.exports = Pool;


function Pool(opts, dom) {
	this.dom = dom;
	dom.navigator = WebKit.navigator;
	this.list = [];
	this.count = 0;
	this.max = opts.max || 8;
	this.over = 0;
	this.maxloads = 100;
	this.notices = 1;
	this.queue = [];
	this.destroyTimeout = opts.destroyTimeout || 600000; // destroy the page if not used for that time
	this.idleTimeout = opts.idleTimeout || 180000; // unload the page if not used for that time
	setInterval(this.wipe.bind(this), Math.max(this.idleTimeout / 4, 10000));
}

function compint(a, b) {
	if (a < b) return -1;
	else if (a == b) return 0;
	else return 1;
}

Pool.prototype.wipe = function() {
	var me = this;
	var now = Date.now();
	var page;
	var list = this.list;
	var nlist = list.length > me.max ? list.sort(function(a, b) {
		var c = compint(a.locked, b.locked); // false first
		if (c) return c;
		c = compint(a.releaseTime, b.releaseTime); // lowest first
		if (c) return c;
		c = compint(b.numloads, a.numloads); // highest first
		return c;
	}) : list;

	nlist = nlist.filter(function(page, index) {
		if (page.locked) return true;
		if (index < nlist.length - me.max || now > page.releaseTime + me.destroyTimeout || page.numloads > me.maxloads) {
			// prevents the page from being grabbed during unload/destroy
			page.locked = true;
			page.unload().catch(error).then(function() {
				return this.destroy();
			}.bind(page)).catch(error);
			return false;
		} else if (page.pingTime && now > page.pingTime + me.idleTimeout) {
			// ensures idling page is unloaded after some time
			page.locked = true;
			page.unload(function(err) {
				this.locked = false;
				error(err);
			}.bind(page));
			return true;
		}
	});

	// in case destroy calls have been made
	if (nlist.length != list.length) {
		debug("Wiped", list.length - nlist.length, "pages");
		this.list = nlist;
		this.count = nlist.length;
	}
};

Pool.prototype.acquire = function(cb) {
	var create = false;
	var page;
	if (this.count < this.max + this.over) {
		this.count++;
		create = true;
	} else {
		for (var i=0; i < this.list.length; i++) {
			page = this.list[i];
			if (!page.locked) {
				break;
			}
			page = null;
		}
	}

	if (page) {
		page.locked = true;
		delete page.releaseTime;
		page.numloads = (page.numloads || 0) + 1;
		page.reset(function(err) {
			page.pingTime = Date.now();
			cb(null, page);
		});
	} else if (create) {
		WebKit(this.dom.settings, function(err, page) {
			if (err) return cb(err);
			page.locked = true;
			this.list.push(page);
			page.prepare = page.preload; // just let us name it prepare
			cb(null, page);
		}.bind(this));
	} else {
		this.queue.push({ts: Date.now(), cb: cb});
		if (this.queue.length > (this.max * this.notices)) {
			this.notices++;
			console.info("express-dom", this.queue.length, "queued acquisitions - consider raising dom.pool.max above", this.max);
		}
	}
};

Pool.prototype.release = function(page) {
	debug("release page", page.uri);
	page.locked = false;
	page.releaseTime = Date.now();
	setImmediate(this.process.bind(this));
};

Pool.prototype.process = function() {
	var next = this.queue.shift();
	if (next) {
		var diff = Date.now() - next.ts;
		if (diff > 50) {
			debug("Queue has been waiting", diff, "milliseconds");
			if (this.count >= this.max) {
				debug("Will allocate extra instance");
				this.over = 1;
			}
		}

		this.acquire(next.cb);

		if (this.over) {
			this.over = 0;
		}
	}
};

function error(err) {
	if (err) console.error(err);
}

