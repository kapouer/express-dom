...
Handler.prototype.middleware = function(req, res, next) {
	...
	if (typeof req == "string") {
		...
		loc = new URL((req.get('X-Forwarded-Proto') || req.protocol) + '://' + req.headers.host + req.url);
		loc.port = req.port;
	}
	// per request settings
	const settings = clone(h.settings);
	settings.location = loc;
	req.query = searchParamsToQuery(loc.searchParams);
...
function searchParamsToQuery(p) {
	const q = {};
