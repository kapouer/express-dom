...
var dom = module.exports = function(model, opts) {
	if (!dom.pool) {
		dom.pool = new Pool(dom.settings.pool); // do not use instance options
		dom.acquire = dom.pool.acquire.bind(pool);
		dom.release = dom.pool.release.bind(pool);
	}
...
Handler.prototype.middleware = function(req, res, next) {
	...
	h.getView(req.app.get('statics'), function(err, data) {
		...
		function between(err) {
			if (err) return next(err);
			if (h.settings.load) {
				h.runMethod('load', state, h.settings.load, finalize);
			} else {
				finalize();
			}
		}
...
Handler.prototype.runMethod = function(method, state, settings, cb) {
	...
	dom.pool.acquire(function(err, page) {
		...
		page[method](state.url, settings);
		delete settings.content;
	});
};

function processMw(page, settings, req, res, next) {
	var list = settings.plugins;
	for (var i=0; i < list.length; i++) {
		list[i](page, settings, req, res, next);
	}
}
