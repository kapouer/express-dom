...
exports.html = function html(page, settings, request, response) {
	page.when('idle', function() {
		...
		if (settings.output) return; // someone has handled this before us
		return page.html();
	}).then(function(str) {
		settings.output = str;
	}).catch(function(err) {
...
exports.cookies = function cookies(page, settings) {
	var cook = req.get('Cookie');
	if (!cook) return;
	settings.cookies = cook;
};
