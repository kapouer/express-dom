...
exports.console = async function (page, { policies, log, scripts}) {
	...
	const secPolChrome = /^Refused to .+ because it violates the following Content Security Policy directive:/;
	const secPolFirefox = /^\[JavaScript Error: "Content-Security-Policy: The page’s settings blocked the loading of a resource /;
	page.on('console', msg => {
		...
		if (type && minLevel <= level) {
			const text = msg.text();
			if (type != "error" || (secPolChrome.test(text) == false && secPolFirefox.test(text) == false)) {
				// eslint-disable-next-line no-console
				console[type](text);
			}
		}
