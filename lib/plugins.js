...
exports.httpequivs = function(page, settings, req, res) {
	page.on('idle', async () => {
		const equivs = await page.evaluate(() => {
			...
			for (const node of document.querySelectorAll('head > meta[http-equiv]')) {
				...
				if (!equivs.has(name)) equivs.set(name, new Set());
				const vals = equivs.get(name);
				for (const str of list) {
					if (vals.has(str) == false) vals.add(str);
				}
				node.remove();
			}
			return equivs;
		});
		const statuses = equivs.get('Status');
		if (statuses) equivs.delete('Status');
		if (statuses?.size) {
			const status = Array.from(statuses).pop();
		...
		for (const [name, equiv] of Object.entries(equivs)) {
			...
			if (typeof header == "string") {
				...
				for (const item of header) equiv.add(item);
			}
			res.set(name, Array.from(equiv).join(','));
		}
	});
};
