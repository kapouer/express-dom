	...
	page.when('idle', function() {
		page.run(function() {
			...
			nodes.forEach(function(node) {
				equivs[node.httpEquiv] = node.content;
				node.remove();
			});
			return equivs;
			...
			if (status) {
				...
				if (!isNaN(code) && [301, 302, 400, 401, 403, 404, 451].indexOf(code) >= 0) {
			...
			Object.keys(equivs).forEach(function(name) {
				response.append(name, equivs[name]);
			});
		});
	});
};
